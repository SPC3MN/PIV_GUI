...
from piv_suite.engines.cpu_engine import _fill_residual_nan
...
def test_fill_residual_nan_replaces_nan_with_valid_median():
    ...
    # residual NaN when a flagged-invalid region sits at the frame edge
    # with no far-side valid neighbor to converge from -- e.g. a
    # no-signal band outside the laser sheet. Left as NaN, that poisons
    # the NEXT pass's RectBivariateSpline fit entirely (the whole
    # interpolated surface goes NaN, not just the bad region), which then
    # crashes scipy.ndimage.map_coordinates with a Windows access
    # violation instead of a Python exception.
